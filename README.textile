...
<code><pre>people_table = Peegee::Table.new(:table_name => 'people')
people_table.ddl # a string containing this table's DDL
people_table.indexes
people_table.foreign_keys
people_table.dependent_foreign_keys</pre></code>
The three commands above return arrays of Peegee::Index, Peegee::ForeignKey and Peegee::DependentForeignKey objects, which all respond to drop and create.
For example, you could drop and recreate the first dependent foreign key (some other table referencing people) with:
<code><pre>people_table.dependent_foreign_keys.first.drop
people_table.dependent_foreign_keys.first.create</pre></code>
...
The strategy used for clustering a table is to store all dependencies in order to restore them later (as viewed above), and move all of the table's data on the order given by a certain index. The result is just like the pgsql native CLUSTER command:
<code><pre>people_table.cluster('people_pk') #must specify an index by which to cluster</pre></code>
